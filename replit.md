...
**Reconnection Strategy**: Scheduled reconnections with configurable delays (default 5 seconds) to prevent reconnection loops. Uses timeout-based scheduling to retry connections after disconnect events, kick events, or connection errors.

**Event-Driven Design**: Leverages Mineflayer's event system for lifecycle management:
- `spawn` event: Confirms successful server join
- `kicked` event: Handles server-initiated disconnections
- `end` event: Triggers reconnection on connection loss
